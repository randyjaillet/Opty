...
Optionable is a searchable, easily-stylable replacement for select elements.

Specific custom markup is required. (It doesn't automatically inject in place of
or transform existing select elements).

As with real select elements, Optionables can:
- be disabled with the "disabled" attribute.
- have default values with the "selected" attribute on options (which are spans here).
- support multiple values with the "multiple" attribute. Optionable handles this
...
Single-select Optionables store their values in corresponding hidden fields
with IDs matching the "data-field-id" attribute of the .optionable.
(Maybe we should just use the same method as with multiple-select Optionables?)

Multiple-select Optionables store their values in a "values" arrays using jQuery's
"data" method on the .optionable.

Terminology used in comments in this file:
- "surface" : The part of the Optionable that is always visible. This is an A tag
...
- "menu" or "dropdown": The sometimes-invisible part of the Optionable containing
the list of options.
- "option": A choosable (unless it's not) list item in the Optionable menu.
	...
	function(){
		...
		$(".optionable").each(
			function(){
				...
				$o.data("shortMode", $(".list span", $o).length < 7);
				...
				if ($(this).is("[disabled]")) {
					...
					// Find a label for the Optionable and make it so clicking
					// the label puts focus on this Optionable's surface.
					var $thislabel = $('[for="' + $o.attr("data-field-id") + '"]');
					if ($thislabel.length) {
						$thislabel.on(
							"click",
							function(e){
								...
								case 13: // Enter key
									...
									if ($o.hasClass("activated")) {
										...
										if ($(".fakefocus",$o).length) {
											...
											// unless this is a multiple-select Optionable, in which case we
											// unselect it.
											// (We're not allowing un-choosing in a single-select Optionable.)
					...
					$(".list span", this).on(
						...
						function(e) {
							if (!$o.is("[disabled]")) {
								...
								// multiple-select Optionable, unselect the option.
								// (We're not allowing un-choosing in a single-select Optionable.)
								...
								// no matter what kind of Optionable this is, as long
		...
		// Whenever anything is clicked, close all the menus except the one
		// that was clicked within if any were in fact clicked within.
		$(document).click(
			function(event){
				var $parentSelect = $(event.target).closest(".optionable");
				hideMenu($(".optionable").not($parentSelect));
			}
		);
...
function showMenu($o) {
	hideMenu($(".optionable").not($o)); // Close other menus.
	if (!$o.hasClass("activated")) {
		$o.addClass("activated");
		$o.closest(".field-row").addClass("optionable-activated");
	}
	if (!$o.data("shortMode")) {
		$(".search",$o).focus();
	}
...
function hideMenu($o, focusOnSurface) {
	if ($o.hasClass("activated")) {
		...
		window.setTimeout(
			function () {
				$o.closest(".field-row").removeClass("optionable-activated");
			},
			300
		)
	}
...
function chooseOption($o, $options, focusOnSurface) {
	...
	// Multiple-select Optionables
	if ($o.is("[multiple]")) {

		// If the Optionable is multiple-select, we're storing the selected
		// values in a "values" array in jQuery's "data" method on the .optionable.
		// If the values array doesn't exist yet for this Optionable, create it.
		if (!$o.data("values"))
		{
			$o.data("values", []);
		}
...
// This function is currently only used on multiple-select Optionables, as
